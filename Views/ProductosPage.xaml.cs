...
namespace AirsoftCore_App.Views
{
    public partial class ProductosPage : ContentPage
    {
        ...
        private void OnAgregarAlCarritoClick(object sender, EventArgs e)
        {
            if (sender is Button button && button.CommandParameter is string productoId)
            {
                var producto = Productos.FirstOrDefault(p => p.Id == productoId);
                AppShell.Carrito.AgregarAlCarrito(producto);
                DisplayAlert("Carrito", $"{producto.Nombre} agregado al carrito", "OK");
            }
        }
